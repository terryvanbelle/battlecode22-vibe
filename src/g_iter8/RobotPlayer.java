...
public strictfp class RobotPlayer {
    ...
    static void runArchon(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        // ITERATION 8: SA_MINERS is cumulative, so once the cap is hit the rule
        // never builds another Miner. Fine in games that end by ~r600 (want pure
        // army then), but on the long r2000 games (chessboard, jellyfish) the
        // sharper Dijkstra army over-commits, our Miners get raided to 0, and
        // the economy never recovers -- we lose the tiebreak. Narrow revival:
        // late game only, if THIS Archon's area has been stripped of Miners and
        // we can afford one.
        int localMiners = 0;
        for (RobotInfo a : rc.senseNearbyRobots(rc.getType().visionRadiusSquared, rc.getTeam()))
            if (a.type == RobotType.MINER) localMiners++;
        boolean needMiners = miners < 6
                || (miners < minerCap() && soldiers + 2 >= miners)
                || (rc.getRoundNum() > 700 && localMiners < 2
                    && rc.getTeamLeadAmount(rc.getTeam()) > 60);
