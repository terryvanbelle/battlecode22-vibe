...
public strictfp class RobotPlayer {
    ...
    static final int MAX_LABS = 2;   // ITERATION 118: team-wide cap, see SA_LAB_BUILT below
    ...
    static void runBuilder(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        if (!builtWatchtower) {
            if (me.isWithinDistanceSquared(home, 8)) {
                Direction best = null; int bestR = Integer.MAX_VALUE;
                for (Direction d : DIRS) {
                    if (!rc.canBuildRobot(RobotType.WATCHTOWER, d)) continue;
                    int r = rc.senseRubble(me.add(d));
                    if (r < bestR) { bestR = r; best = d; }
                }
                if (best != null) {
        ...
        // committed investment, always worth pursuing), a second Lab is
        // discretionary surplus spending -- only worth the 7-tile detour if
        // the team is already lead-rich (checked once, before committing to
        // the walk, not worth camping and waiting for lead that may never
        // come on lead-scarce maps).
        if (!builtLab && rc.readSharedArray(SA_LAB_BUILT) >= 1
                && rc.getTeamLeadAmount(rc.getTeam()) < 1500) {
            builtLab = true;
            rc.setIndicatorString("skipping 2nd lab, not lead-rich enough");
        }
        if (!builtLab) {
            ...
            Direction best = null; int bestR = Integer.MAX_VALUE;
            for (Direction d : DIRS) {
                if (!rc.canBuildRobot(RobotType.LABORATORY, d)) continue;
                int r = rc.senseRubble(me.add(d));
                if (r < bestR) { bestR = r; best = d; }
            }
            if (best != null) {
        ...
        // permanently wasted. Bounded to +1 per Builder (not unlimited) so this
        // scales with however many Builders richHome (Iteration 109) already
        // allows, rather than spamming without limit.
        if (extraWatchtowers < 1 && rc.getTeamLeadAmount(rc.getTeam()) > 1000
    ...
    static void runWatchtower(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        if (target != null && rc.canAttack(target.location)) {
            ...
            rc.setIndicatorString("watchtower attack " + target.type);
            return;
        }
        boolean localThreat = false;
        for (RobotInfo e : foes) if (combat(e.type) && e.location.isWithinDistanceSquared(rc.getLocation(), 40)) { localThreat = true; break; }
