...
public strictfp class RobotPlayer {
    ...
    static void runArchon(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        int builderCap = rc.getRoundNum() > 400 ? 2 : 1;
        boolean needBuilder = myBuildersSpawned < builderCap && rc.getRoundNum() > 100
                && miners >= 8 && rc.getTeamLeadAmount(rc.getTeam()) > 300;
    ...
    static void runBuilder(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        if (!builtLab) {
            ...
            Direction away = home.directionTo(me);
            if (away != Direction.CENTER && rc.isMovementReady() && rc.canMove(away)) rc.move(away);
        ...
        // check above is naturally in range of it too -- unlike the
        // Laboratory (built 7 tiles out), which would need its own travel
        // logic and isn't attempted this iteration.
        for (RobotInfo r : rc.senseNearbyRobots(rc.getType().actionRadiusSquared, rc.getTeam())) {
            if (r.type == RobotType.WATCHTOWER && rc.canMutate(r.location)) {
                rc.mutate(r.location);
                rc.setIndicatorString("mutate watchtower");
                return;
            }
        }
