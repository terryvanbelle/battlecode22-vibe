...
public strictfp class RobotPlayer {
    ...
    static final int SA_SAGE_SEEN = 38, SA_SAGE_SEEN_RND = 39;  // ITERATION 96: packed loc +
                                                ...
                                                // converged on)
    // ITERATION 127: bytecode-budget monitoring, requested directly by the
    // user (a silent bytecode overrun -- the engine truncates a robot's
    // turn mid-instruction and resumes next round -- has apparently killed
    // strategies in past tournaments, and a one-off offline profiler check
    // this project already did once, long ago, found comfortable headroom
    // on the game state of that moment; this makes the check live, ongoing,
    // and self-reporting instead of a single point-in-time snapshot).
    // SA_BYTECODE_NEARMISS: rounds where a robot used >=85% of its type's
    // bytecode budget but didn't overrun. SA_BYTECODE_OVERRUN: rounds that
    // actually spilled into the next round (confirmed via the round-number
    // check in run() below, not just a usage-percentage guess).
    static final int SA_BYTECODE_NEARMISS = 40, SA_BYTECODE_OVERRUN = 41;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                checkBytecodeBudget(rc, roundAtStart);
                Clock.yield();
            }
        }
    }

    // ITERATION 127: see the SA_BYTECODE_NEARMISS/SA_BYTECODE_OVERRUN
    // declaration above for the why. Two independent signals:
    //  - OVERRUN (definitive): if `rc.getRoundNum()` has moved past
    //    `roundAtStart` by the time we reach here, this robot's execution
    //    was paused mid-instruction by the engine's bytecode limiter and
    //    resumed on a later round -- an actual, confirmed truncation, not
    //    a guess. (Battlecode's bytecode limiter does not throw an
    //    exception or reset the call stack; it pauses execution at the
    //    exact instruction pointer and resumes there next round, so any
    //    code after the truncation point genuinely never ran this round.)
    //  - NEAR-MISS (early warning): `Clock.getBytecodeNum()` vs. this
    //    robot's own `bytecodeLimit`, checked when no overrun occurred --
    //    >=85% usage flags a robot that's close enough to the edge that a
    //    slightly more expensive game state (more units in vision, a
    //    longer Dijkstra search, etc.) could tip it into a real overrun.
    // Wrapped in its own try/catch: a bug in the monitoring code itself
    // must never be able to crash the robot's main loop, which would be
    // strictly worse than the problem it's trying to catch.
    static void checkBytecodeBudget(RobotController rc, int roundAtStart) {
        try {
            if (rc.getRoundNum() != roundAtStart) {
                bump(rc, SA_BYTECODE_OVERRUN);
                System.out.println(rc.getType() + " #" + rc.getID()
                        + " BYTECODE OVERRUN: started round " + roundAtStart
                        + ", resumed round " + rc.getRoundNum());
                return;
            }
            int limit = rc.getType().bytecodeLimit;
            if (limit <= 0) return;
            int used = Clock.getBytecodeNum();
            if (used * 100 >= limit * 85) bump(rc, SA_BYTECODE_NEARMISS);
        } catch (Exception e) {
            // never let monitoring itself break the robot's turn
        }
    }
    ...
    static void report(RobotController rc) throws GameActionException {
        ...
        // ITERATION 127: this string was already at/near Battlecode's
        // indicator-string length limit before adding bytecode tracking --
        // confirmed via a live game that it was silently truncating mid-word
        // ("attacks=" cut to "atta"), so nothing appended after that point
        // was ever visible. Compacted every field to short abbreviations and
        // moved the new bytecode counters (bcN/bcO) right after the round
        // number specifically so they survive truncation even if the
        // overall budget is still tight in some game state this doesn't
        // cover.
        rc.setIndicatorString(
            "r" + rc.getRoundNum()
            + " bcN" + rc.readSharedArray(SA_BYTECODE_NEARMISS)
            + " bcO" + rc.readSharedArray(SA_BYTECODE_OVERRUN)
            + " L" + rc.getTeamLeadAmount(me) + " G" + rc.getTeamGoldAmount(me)
            + " A" + rc.getArchonCount() + " HP" + rc.getHealth()
            + " M" + rc.readSharedArray(SA_MINERS) + " S" + rc.readSharedArray(SA_SOLDIERS)
            + " atk" + rc.readSharedArray(SA_ATTACKS) + " eA" + ea
            + " HT" + (rc.readSharedArray(SA_HOME_THREAT) != 0 ? 1 : 0)
        );
    }
}
