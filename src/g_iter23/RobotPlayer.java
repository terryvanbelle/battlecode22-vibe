...
public strictfp class RobotPlayer {
    ...
    static void runArchon(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        boolean needBuilder = myBuildersSpawned < 1 && rc.getRoundNum() > 100
                && miners >= 8 && rc.getTeamLeadAmount(rc.getTeam()) > 300;
        RobotType want = needBuilder ? RobotType.BUILDER
    ...
    static void runBuilder(RobotController rc, RobotInfo[] foes) throws GameActionException {
        ...
        if (!me.isWithinDistanceSquared(home, 8)) moveToward(rc, home);
    ...
    static int targetPriority(RobotType t) {
        if (combat(t)) return 2;
        if (t == RobotType.ARCHON) return 1;
        return 0;   // Miner, Builder, Laboratory
    }
