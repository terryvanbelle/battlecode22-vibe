...
// Vendored verbatim (package rename only) from jmerle/battlecode-2022
// (camel_case_v25_final), MIT licence (c) 2022 Jasper van Merle.
...
//
// Unrolled within-vision (radius^2 <= 20) Dijkstra: from the robot's tile it
// relaxes shortest paths to every visible tile with cost 1 + rubble, then
// getBestDirection(target, blockedDirection) returns the first step of the
// shortest path toward `target`. blockedDirection (usually the reverse of the
// last move) is excluded so the robot doesn't oscillate against an obstacle.
